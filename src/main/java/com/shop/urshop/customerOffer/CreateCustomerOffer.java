...
public class CreateCustomerOffer {
  ...
  public CreateCustomerOffer(
          CustomerOfferRepository customerOfferRepository, ProductRepository productRepository, ProductCustomerOfferMapRepository productCustomerOfferMapRepository) {
  ...
  public Set<Product> getProducts() {
    ...
    return products.stream()
            .filter(product -> product.getStock() < 3)
    ...
    for (Product product:products ) {
      ...
      if (Objects.nonNull(map)){
        ...
        if (totalAmount!=map.getCustomerOffer().getTotalAmount()) {
          customerOfferRepository.updateTotalAmountById(totalAmount,map.getCustomerOffer().getId());
          System.out.println("güncellendi");
        }
      }else{
        ProductCustomerOfferMap productCustomerOfferMap = ProductCustomerOfferMap.builder()
                .customerOffer(customerOffer)
                .product(product)
                .build();
